...
function injectSEO(html, target) {
    ...
    // 1. Remove ANY naked JSON (Double Layer Protection)
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
  <url><loc>https://mapledevs.ca/</loc><changefreq>daily</changefreq><priority>1.0</priority></url>`;
    ...
    for (const job of jobs) {
        ...
        const target = { folder: `jobs/${slug}`, hash: `#id=${slug}`, title: `${job.title} - ${job.studio}`, desc: `Apply for ${job.title} at ${job.studio}.` };
        fs.writeFileSync(path.join(jobPath, 'index.html'), injectSEO(baseHTML, target));
