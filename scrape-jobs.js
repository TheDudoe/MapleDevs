  ...
  try {
    ...
    return jobs
      .filter(job => {
        ...
        return CANADA_KEYWORDS.some(kw => loc.includes(kw)) || (studio.locationFilter && text.includes(studio.locationFilter.toLowerCase()));
      })
      .map(job => ({
        title: job.text || '',
        studio: studio.name,
...
async function scrapeSmartRecruiters(studio) {
  ...
  try {
    ...
    return jobs
      .filter(job => {
        const loc = (job.location?.city || job.location?.country || '').toLowerCase();
        return CANADA_KEYWORDS.some(kw => loc.includes(kw));
      })
      .map(job => ({
        title: job.name || '',
        studio: studio.name,
        location: `${job.location?.city || ''}, ${job.location?.region || ''}`.trim().replace(/^,|,$/g, '') || studio.city,
        type: guessJobType(job.name, ''),
        mode: guessWorkMode(job.name, job.location?.city || '', ''),
  ...
  try {
    ...
    return jobs
      ...
      .map(job => ({
        title: job.title || '',
        studio: studio.name,
        location: job.location || studio.city,
        type: job.employmentType || guessJobType(job.title, job.description || ''),
        mode: guessWorkMode(job.title, job.location || '', job.description || ''),
        ...
        try {
          ...
          const processed = jobs
            .filter(job => {
              ...
              const isCanada = CANADA_KEYWORDS.some(kw => loc.includes(kw));
              const matchesFilter = !studio.locationFilter || title.includes(studio.locationFilter.toLowerCase()) || loc.includes(studio.locationFilter.toLowerCase());
              return isCanada && matchesFilter;
            })
            .map(job => ({
              title: job.title || '',
              studio: studio.name,
              location: job.locationsText || studio.city,
              type: guessJobType(job.title, ''),
              mode: guessWorkMode(job.title, job.locationsText || '', ''),
